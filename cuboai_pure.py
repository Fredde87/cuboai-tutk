...
def transcode(plain: bytes, swap_tail: bool = True) -> bytes:
    ...
    Maps the real plaintext (e.g. the av-connect, which carries the account and
    password in full 16-byte blocks) to the exact bytes that go on the wire.

    The trailing `len & 0xF` bytes are XOR'd with K16; additionally, for the
    post-connect DATA-channel frames (IOCTLs, AV, data-acks, resends) TUTK applies a
    `Swap` byte-permutation for tail lengths 2/4/8: `wire_tail = Swap(plain_tail XOR
    K16)`. This is byte-identical to the native library and is what makes a
    lullaby-schedule SET (duration field in an 8-byte tail) store correctly.

    `swap_tail=False` reproduces the NO-Swap wire of the pre-session SEARCH frames
    (probe / search-ack / lan-query), which native sends via a path that does not run
    the Swap. Direct-send frames (close, keepalive) never swap either and use
    `xor_frame`. Tails of len 6 (av-connect) / 12 (IOCTL GET) are Swap-identity.
    """
    n = len(plain)
    full = n - (n & 0xF)
    out = bytearray(n)
...
# TransCodePartial / ReverseTransCodePartial permute the partial-block tail (a `Swap`)
# for tail lengths 2/4/8 only (identity otherwise): encode `wire_tail =
# Swap(plain_tail XOR K16)`, decode `plain_tail = Swap(wire_tail) XOR K16` (Swap is an
# involution). Verified byte-for-byte against the native library for all lengths.
_TAIL_SWAP = {2: (1, 0), 4: (2, 3, 0, 1), 8: (7, 4, 3, 2, 1, 6, 5, 0)}


def _tail_swap(buf, length):
    """The TransCodePartial tail `Swap` permutation (identity unless length is 2/4/8;
    an involution). Used by both `transcode` and `inv_transcode` so encode/decode match
    native TransCodePartial / ReverseTransCodePartial exactly."""
...
def inv_transcode(wire: bytes) -> bytes:
    ...
    ReverseTransCodePartial. The tail (len & 0xF == 2/4/8) is un-Swapped then XOR'd:
    `plain_tail = Swap(wire_tail) XOR K16` (Swap is an involution, so the same
    `_tail_swap` inverts the encode).
    """
    n = len(wire)
    full = n - (n & 0xF)
    out = bytearray(n)
...
class TUTKDirectSession:
    ...
    def add_lullaby_schedule(self, name, *, song=None, uuid=None, days_mask=0x7f,
        ...
        The 148-byte payload makes a 216-byte frame with an 8-byte tail carrying the
        duration field; `transcode` applies the TransCodePartial tail `Swap` for tail
        2/4/8 (as native does), so the duration stores exactly as sent."""
        import cuboai_messages as cm
