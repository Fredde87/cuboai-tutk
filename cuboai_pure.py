...
_AVMID_FALLBACK = bytes.fromhex("000000000000")   # this-VM value; used iff MAC read fails
...
def compute_av_mid():
    ...
    (Linux) → uuid.getnode() (Windows + universal). Falls back to this-VM's historical
    value only if every method fails, so import never raises; a stderr note is emitted
    on that final fallback. On Linux/macOS getifaddrs wins, so behaviour is unchanged.
    """
